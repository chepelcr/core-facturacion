    ...
    if (formulario != '') {
        ...
        Pace.track(function () {
            $.ajax({
                ...
                if(jqXHR.status == 500){
                    notificacion('Error de servidor', '',  'error');
                }

                else{
                    if(errorThrown == 'login')
                    {
                        //Iniciar sesion enviando la funcion y la data
                        iniciar_sesion(function(){
                            enviar_frm(formulario, nombre_elemento);
                        });

                        notificacion('Debe iniciar sesion para continuar', '',  'warning');
                    }
                    else{
                        notificacion(errorThrown, '',  'error');
                    }
                }
            });
        });
